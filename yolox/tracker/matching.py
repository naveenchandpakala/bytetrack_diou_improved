...
def _diou_matrix(atlbrs, btlbrs):
    """Compute DIoU-based cost for tlbr boxes."""
    M = len(atlbrs)
    N = len(btlbrs)
    ...
    for i in range(M):
        ...
        for j in range(N):
            ...
            center_dist = (acx - bcx) ** 2 + (acy - bcy) ** 2
            enc_x1 = min(ax1, bx1)
            enc_y1 = min(ay1, by1)
            enc_x2 = max(ax2, bx2)
            enc_y2 = max(ay2, by2)
            ...
            cost[i, j] = np.clip((1.0 - diou) / 2.0, 0.0, 1.0)
...
def diou_distance(atracks, btracks):
    ...
    if (len(atracks)>0 and isinstance(atracks[0], np.ndarray)) or (len(btracks) > 0 and isinstance(btracks[0], np.ndarray)):
        ...
        atlbrs = [track.tlbr for track in atracks]
        btlbrs = [track.tlbr for track in btracks]
    atlbrs = np.asarray(atlbrs, dtype=float).reshape(-1, 4)
    btlbrs = np.asarray(btlbrs, dtype=float).reshape(-1, 4)
    return _diou_matrix(atlbrs, btlbrs)
